...
export default function ErrorLogs() {
  ...
  const [activeTab, setActiveTab] = useState('records') // 'records' | 'patterns'
  ...
  const { data: patternsData, isLoading: patternsLoading } = useQuery({
    queryKey: ['error-log-patterns'],
    queryFn: () => errorLogService.patterns({ days: 7, limit: 50 }),
  ...
  const patterns = patternsData?.data?.data || []
  ...
  const filterByErrorClass = (errorClass) => {
    setSearchInput(errorClass)
    setFilters(f => ({ ...f, search: errorClass, page: 1 }))
  ...
  const diagnosisData = detail?.metadata?.diagnosis || null
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => {
              queryClient.invalidateQueries({ queryKey: ['error-logs'] })
              queryClient.invalidateQueries({ queryKey: ['error-log-stats'] })
              queryClient.invalidateQueries({ queryKey: ['error-log-patterns'] })
            }}
          >
          ...
          <CardContent className="p-0">
            {patternsLoading ? (
              ...
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-sm">No se encontraron patrones en los ultimos 7 dias</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Clase de error</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Total</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Sin resolver</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Severidades</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tipos</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Primera vez</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ultima vez</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {patterns.map((pattern) => (
                      <tr key={pattern.error_class} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm font-mono text-gray-900 max-w-[250px] truncate">
                          {pattern.error_class}
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-sm font-medium ${pattern.unresolved_count > 0 ? 'text-orange-600' : 'text-green-600'}`}>
                            {pattern.unresolved_count}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {(pattern.severities || []).map(s => (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {(pattern.event_types || []).map(t => (
                              <EventTypeBadge key={t} eventType={t} />
                            ))}
                          </div>
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">
                        ...
                        <td className="px-4 py-3">
                          <Button
                            variant="secondary"
                            size="sm"
                            onClick={() => filterByErrorClass(pattern.error_class)}
                          >
      ...
      >
        {detail && (
          <div className="space-y-6">
            ...
            {diagnosisData && (
              <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-4 border border-indigo-100">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="text-sm font-semibold text-indigo-900">Diagnostico</h4>
                </div>
                <div className="space-y-3">
                  {/* Priority + Category */}
                  <div className="flex items-center gap-3 flex-wrap">
                    <PriorityBadge priority={diagnosisData.priority} />
                    {diagnosisData.category && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-700">
                        {diagnosisData.category}
                      </span>
                    )}
                    {diagnosisData.auto_resolved && <AutoResolvedBadge />}
                  </div>

                  {/* Similar count */}
                  {diagnosisData.similar_count_24h > 0 && (
                    <p className="text-sm text-indigo-700">
                      <span className="font-medium">{diagnosisData.similar_count_24h}</span> errores similares en las ultimas 24 horas
                      <button
                        className="ml-2 text-indigo-600 underline hover:text-indigo-800 text-xs"
                        onClick={() => {
                          filterByErrorClass(detail.error_class)
                          setSelectedLog(null)
                        }}
                      >
                        Ver todos
                  ...
                  {/* Auto-resolve reason */}
                  {diagnosisData.auto_resolved && diagnosisData.auto_resolve_reason && (
                    <div className="flex items-start gap-2 bg-teal-50 rounded p-2 border border-teal-200">
                      <Sparkles className="w-4 h-4 text-teal-600 mt-0.5 shrink-0" />
                      <p className="text-sm text-teal-800">
                        <span className="font-medium">Auto-resuelto:</span> {diagnosisData.auto_resolve_reason}
                      </p>
                    </div>
                  )}

                  {/* Suggested actions */}
                  {diagnosisData.suggested_actions?.length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-indigo-700 mb-1.5">Acciones sugeridas:</p>
                      <ol className="list-decimal list-inside space-y-1">
                        {diagnosisData.suggested_actions.map((action, i) => (
                          <li key={i} className="text-sm text-indigo-800">{action}</li>
                        ))}
                      </ol>
                    </div>
                  )}
            ...
            {/* Metadata (excluding diagnosis which is shown separately) */}
            {detail.metadata && Object.keys(detail.metadata).filter(k => k !== 'diagnosis').length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-1">Metadata</h4>
                <pre className="text-xs text-gray-700 bg-gray-50 rounded-lg p-3 overflow-auto max-h-40">
                  {JSON.stringify(
                    Object.fromEntries(Object.entries(detail.metadata).filter(([k]) => k !== 'diagnosis')),
                    null, 2
                  )}
                </pre>
              </div>
